(function () {
  ...
  function validQuestionIds(ids) {
    ...
    return (ids || []).filter((id) => bankIds.has(id));
  }
  ...
  function renderDashboard() {
    ...
    const resumeProgress = resume
      ? `上次练习：${escapeHtml(state.resume.title || "练习")}，第 ${Math.min((state.resume.index || 0) + 1, state.resume.ids.length)} / ${state.resume.ids.length} 题`
      : "";
    ...
    page(
      ...
      <div class="hero">
        ...
        <div class="hero-actions">
          <button class="primary" data-action="${resume ? "resume" : "sequence"}">${resume ? "继续上次练习" : "开始顺序练习"}</button>
          <button class="secondary" data-open="random">随机抽题</button>
        </div>
        ${resume ? `<p class="subtle" style="color:#ffd1cc;margin-top:12px">${resumeProgress}</p>` : ""}
      </div>
  ...
  function startPractice(kind, ids, title, resumeSaved = false) {
    clearTimer();
    simulation = null;
    const saved = resumeSaved ? state.resume : null;
    const activeIds = validQuestionIds(saved?.ids?.length ? saved.ids : ids);
    const activeKind = saved?.kind || kind;
    const activeTitle = saved?.title || title;
    if (!activeIds.length) {
    ...
    session = {
      ...
      ids: activeIds,
      title: activeTitle,
      index: Math.min(saved?.index || 0, activeIds.length - 1),
      answered: false,
      result: null,
      selected: [],
      response: "",
  ...
  function resumePractice() {
    if (!state.resume?.ids?.length) {
      startPractice("sequence", questionIds(), "顺序练习");
      return;
    }
    startPractice(state.resume.kind, state.resume.ids, state.resume.title, true);
  }
  ...
  app.addEventListener("click", (event) => {
    ...
    if (target.dataset.action === "resume") resumePractice();
    if (target.dataset.action === "sequence") startPractice("sequence", questionIds(), "顺序练习");
